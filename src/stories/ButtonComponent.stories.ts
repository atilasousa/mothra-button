...
export default {
  ...
  component: ButtonComponent,
    argTypes: {
      ...
      icon: {
        description:"Button Icon, i'have created a select input with two options just for tests purposes",
        control: { type: "select" },
        options: [`<font-awesome-icon icon="user-secret" />`, `<font-awesome-icon icon="house" />`,],
      },
      onClick: {
        action: "click",
      },
...
const Template = (args:any,{argTypes}:any) => ({
    ...
    template: `<button-component @click="click" v-bind="args">
        <template v-if="${'label' in args}" v-slot:label>${args.label}</template>
